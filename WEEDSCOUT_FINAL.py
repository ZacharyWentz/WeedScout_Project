...
# Updated 11/23/2025 — Continuous video, snapshot saving, object GPS labeling
...
class UserAppCallbackWithGPS(app_callback_class):
    ...
    def __init__(self, gps_obj, mav_master):
        ...
        with open(self.csv_file, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(["ID#", "Frame#", "Timestamp", "ObjLat","ObjLon","DroneLat","DroneLon","DroneAlt"])
...
snapshot_queue = queue.Queue(maxsize=200)
def detection_processing_thread(user_data):
    ...
    while preview_running or not user_data.detection_queue.empty():
        ...
        frame_targets = [d for d in detections if d.get_label().lower() == user_data.TARGET_CLASS.lower()]
        ...
        for det in frame_targets:
            ...
            with user_data.lock:
                for oid, tracked in user_data.tracked_objects.items():
                    if tracked.get('lat') is None or obj_lat is None:
                        continue
                    if math.hypot(obj_lat-tracked['lat'], obj_lon-tracked['lon']) < user_data.redetect_distance:
                        matched_id = oid
                ...
                if not tracked['snapshot_taken']:
                    ...
                    cv2.putText(snapshot_frame,
                                f"Time: {datetime.now(pytz.timezone('US/Central')).strftime('%H:%M:%S')}",
                                (10,25), cv2.FONT_HERSHEY_SIMPLEX,0.7,(255,255,255),2)
                    drone_text = f"Drone: Lat {lat if lat else 'N/A'}, Lon {lon if lon else 'N/A'}, Alt {alt if alt else 'N/A'} m"
                    cv2.putText(snapshot_frame, drone_text, (10,55), cv2.FONT_HERSHEY_SIMPLEX,0.7,(255,255,255),2)
                    try:
            ...
            cv2.putText(frame_overlay,
                        f"ID {matched_id} | Lat {lat_str} Lon {lon_str}",
                        (x_min, max(20,y_min-10)),
                        cv2.FONT_HERSHEY_SIMPLEX,0.5,(0,255,0),2)

        gps_text = f"Drone: Lat {lat if lat else 'N/A'}, Lon {lon if lon else 'N/A'}, Alt {alt if alt else 'N/A'} m"
        cv2.putText(frame_overlay, f"Time: {datetime.now(pytz.timezone('US/Central')).strftime('%H:%M:%S')} | Objects detected: {len(frame_targets)}",
                    (10,25), cv2.FONT_HERSHEY_SIMPLEX,0.7,(255,255,255),2)
...
def snapshot_writer_thread(user_data):
    while preview_running or not snapshot_queue.empty():
        try:
            item = snapshot_queue.get(timeout=0.05)
        except queue.Empty:
            continue
        ...
        try:
            ...
            with open(user_data.csv_file, 'a', newline='') as f:
                ...
                writer.writerow([
                    ...
                    item.get('drone_lat',"N/A"),
                    item.get('drone_lon',"N/A"),
                    item.get('drone_alt',"N/A")
                ])
        except Exception:
